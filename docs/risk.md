...
- `User.organisationId/role/teamId/isLineManager` columns are still written by signup as a PM6 rollback fallback. They are not read by any code path, but they exist on disk and could be re-trusted by a future regression. PM6 drops them.
...
| P25 | Admin management routes must write `role`/`teamId`/`isLineManager` to `Membership`, never to `User` — enforced by convention and tests only, not by the type system | After PM3-COMPLETE (ADR-027), `Membership` is the data-plane source of truth for role and team assignment. However, `User.role`, `User.teamId`, and `User.isLineManager` still exist in the schema. A future route author who writes `db.user.update({ data: { role: "MANAGER" } })` will not get a compile error — the columns are valid Prisma targets. The correct table choice is enforced only by the forge test suite (`pm3-member-management-membership.test.ts`) and code-review convention. | Source: `src/app/api/admin/members/[id]/{promote,demote,role,assign-to-team,remove-from-team}/route.ts`. Mitigation: PM6 migration to drop `User.role`, `User.teamId`, `User.isLineManager` columns will convert this from a convention risk to a compile-time guarantee. |
